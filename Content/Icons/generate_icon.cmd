...
set position=+0+20
set text=Settings\nCopier
set overlay=SettingsCopier-Icon.png
set targetName=SettingsCopier

convert -verbose "%srcimg%" -font "%font%" -pointsize %fontSize% -stroke "%stroke%" -strokewidth 2 -fill "%fill%" -gravity center -annotate "%position%" "%text%" -resize 256x256 SettingsCopier-icon-256.png
convert -verbose "%srcimg%" -font "%font%" -pointsize %fontSize% -stroke "%stroke%" -strokewidth 1 -fill "%fill%" -gravity center -annotate "%position%" "%text%" -resize 64x64 SettingsCopier-icon-64.png
